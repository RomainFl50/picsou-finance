...
Accounts created by a Finary import have no `Requisition` for the above backfill to attach a logo to, even when their `provider` name matches a real Enable Banking institution (e.g. a Finary-imported "Boursorama Banque" checking account). `SyncService.backfillAccountLogosByProvider()` covers this case directly near the end of the daily scheduler (`SchedulerService.dailyBankSync()`), after account-producing syncs have completed. It positively selects the `finary_` external-id namespace, with a non-null `provider`, no `logoUrl`, and no `Account.logoBackfillAttemptedAt`, then re-runs the same `searchInstitutions` + `findInstitution` match used by `ensureLogoUrl`. This avoids redundant full-catalog calls for requisition-backed accounts and providers that structurally have no Enable Banking logo source. There is no institutionId on a bare Finary account (no country hint), so the search is unscoped across all countries; `findInstitution` still falls back to its case-insensitive name match. The work is bounded to a single attempt per account via `Account.logoBackfillAttemptedAt`, same pattern as the requisition-level backfill. Providers with no presence in Enable Banking's catalog at all (Trade Republic, screen-scraped Bourso PEA/LEP, crypto wallets/exchanges) still show the color fallback.
...
- `backend/src/main/resources/db/migration/V56__account_logo_backfill_attempted.sql`
...
- `backend/src/main/resources/db/migration/V57__family_member_show_bank_logos.sql`
